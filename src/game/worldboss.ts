...
export interface WbReport {
   ...
   */
  restarted: boolean
  /** ok=false 时的原因：'cooldown' | 'defeated' | 'period ended' | 'not-exhausted' | 'settling' */
  reason: string
}
 ...
 */
export function parseWorldBoss(raw: unknown): WbState {
  ...
  return {
    ...
    boss: {
      ...
      form: Math.min(BOSS_FORM_MAX || 1, Math.max(1, num(b.form) || 1)),
...
export async function reportDamage(damage: number): Promise<WbReport> {
  const res = await fetch(API, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ playerId: getPlayerId(), damage: Math.max(0, Math.floor(damage)) }),
  })
  if (!res.ok) throw new Error(`HTTP ${res.status}`)
 ...
 *
 * ⚠️ **这里不扣钱**：这个模块刻意不 import engine（反向依赖整个 engine 会让它没法单独测），
 *    扣费由调用它的界面负责，而且**必须先请求、后扣费** —— 服务端说不行（没打满 / 还在结算）
 *    的时候一分钱都不该动。失败就把钱退回去，别留下"点了没反应还扣了钱"。
 *
...
/** 把一笔伤害攒进队列并安排发送 */
export function queueMatchDamage(damage: number): void {
  if (!(damage > 0)) return
  queued += damage
  emit()
  scheduleFlush()
...
async function flushQueued(): Promise<void> {
  ...
  try {
    ...
    if (!r.ok && r.accepted <= 0) giveBack = d
  } catch (e) {
